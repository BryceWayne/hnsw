...
```
Persistence functions. `SaveToWriter` and `LoadFromReader` allow integration with any storage backend (AWS S3, Google Cloud Storage, Azure Blob Storage, etc.) by implementing the standard `io.Reader` and `io.Writer` interfaces.

See [examples/gcp](examples/gcp) for a demonstration of saving/loading with Google Cloud Storage.
